class User < ApplicationRecord
  ...
  SENSITIVE_SERIALIZATION_FIELDS = [ :private_key, :private_key_encrypted ].freeze
  ...
  def as_json(options = {})
    super(sanitized_serialization_options(options))
  end
  ...
  def sanitized_serialization_options(options)
    sanitized = (options || {}).dup
    sanitized.delete(:include_private_key)
    excluded = Array(sanitized[:except]) + SENSITIVE_SERIALIZATION_FIELDS
    sanitized[:except] = excluded.uniq
    sanitized
  end
